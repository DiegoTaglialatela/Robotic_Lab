...
namespace gazebo
{
  class EX8Plugin : public ModelPlugin {
  ...
  GZ_REGISTER_MODEL_PLUGIN(EX8Plugin)
...
void EX8Plugin::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf) {
    _nh = new ros::NodeHandle();	
    model = _parent;
    _pos_sub = _nh->subscribe("/pose", 0, &EX8Plugin::pos_cb, this);
    _js_sub = _nh->subscribe("/lbr_iiwa/joint_states", 0, &EX8Plugin::joint_states_cb, this);
    this->updateConnection = event::Events::ConnectWorldUpdateBegin(std::bind(&EX8Plugin::OnUpdate, this));
    if(!_initIK())
        exit(1);
...
void EX8Plugin::pos_cb( geometry_msgs::PoseConstPtr pose){
    ...
    cout << "Posizione desiderata: " << _des_frame.p.x() << " " << _des_frame.p.y()
            << " " << _des_frame.p.z() << endl;
    cout << "Orientamento desiderato: " << pose->orientation.x <<  " " << pose->orientation.y
            << " " << pose->orientation.z << " " << pose->orientation.w << endl;
}


void EX8Plugin::joint_states_cb(sensor_msgs::JointState js) {
	for(int i=0; i<_k_chain.getNrOfJoints(); i++ ) 
		_q->data[i] = js.position[i];
}


void EX8Plugin::OnUpdate(){
